...
// 죽고, 그 사유(«... is not allowed in 1hyok/SlowClock»)는 job 이 만들어지지 않아
// 로그에도 남지 않는다 — 관리자만 설정 화면에서 확인할 수 있다. 그 침묵을 막으려고
// 아래 테스트가 diff 옆에서 먼저 red 를 낸다.
//
// 실측값 (GET /repos/1hyok/SlowClock/actions/permissions[/selected-actions]):
...
test('dependency audit resolves the app runtime graph and runs the compatibility gate', async () => {
  const source = await readFile(
    new URL('../workflows/dependency-audit.yml', import.meta.url),
    'utf8',
  );

  // :app 의 debug runtime classpath 에 core/feature 모듈의 런타임 의존이 전부 들어온다. 순수 JVM
  // 모듈이 생기면 그 모듈의 runtimeClasspath 를 따로 뽑아야 하므로 빌드 스크립트에서 판정한다.
  ...
  assert.ok(
    source.includes(':app:dependencies --configuration debugRuntimeClasspath'),
    ':app:dependencies --configuration debugRuntimeClasspath is missing',
  );
  modules
    .filter(({ android }) => !android)
    .forEach(({ projectPath }) => {
      const command = `${projectPath}:dependencies --configuration runtimeClasspath`;
...
test('dependency graph generation is immutable, fail closed, and wrapper validated', async () => {
  ...
  // PR 생성 job 하나 + main push 제출 job 하나. develop 에서 main 기준선을 손으로 제출하던 job 은
  // main 단일 흐름에서 필요 없다.
...
test('Dependabot updates Actions and the screenshot image but not Gradle versions', async () => {
  ...
  // 이 저장소의 분류 라벨은 Task/Bug/Feature 셋뿐이다 — 없는 라벨을 적으면 Dependabot 이 경고만 남긴다.
  assert.equal(source.match(/^\s+- Task$/gm)?.length, 2);
