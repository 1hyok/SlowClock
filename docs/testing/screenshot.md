...
무엇을 캡처할지는 워크플로가 화면을 탐색해서 추측하지 않는다. 각 모듈의 `src/screenshotTest/**/*ScreenshotTest.kt` 가 `@PreviewTest` 로 Preview 함수와 상태를 선언하며, 워크플로는 그 테스트 전체를 실행한다. 새 화면·새 상태를 추가하려면 먼저 screenshot test 를 추가한다. 생성된 이미지는 PR 의 PNG diff 에서 눈으로 최종 확인한다.
