...
아래 계약은 온라인 쓰기 구현을 포함한 새 앱 버전에 적용된다. 구버전 앱에 남은 오프라인 쓰기 대기열을 제거하거나, 이미 대기 중인 요청을 같은 ID 재시도로 바꾸지는 않는다. 구버전 잔존 여부와 재연결 후 기존 요청의 처리를 출시 검증에 포함한다.
...
2026-09-06 작업 기준으로 운영 규칙은 배포하지 않았으며, 운영 공유 등록부의 inventory와 충돌 정리도 아직 확인하지 않았다. 저장소·에뮬레이터 검증 통과를 운영 전환 완료로 간주하지 않는다.

1. 기존 사용자·일정의 공유 코드와 `shareCodes` 소유 관계를 점검한다. 누락·중복·소유자 불일치를 구분하고, 기존 사용자의 코드가 바뀌거나 다른 소유자의 등록부를 덮지 않도록 충돌을 정리한다. 새 앱의 자동 복구만으로 구버전 사용자의 등록부까지 정리됐다고 가정하지 않는다.
2. 점검·정리 결과를 확인한 뒤 아래 명령으로 새 규칙을 적용한다. 새 앱을 구규칙에 먼저 연결하면 등록부 동일값 확인이나 첫 transaction의 미존재 일정 조회가 거절될 수 있다. 반대로 등록부가 맞지 않는 구버전 앱은 새 규칙 아래 공유 일정 생성·편집·완료가 거절될 수 있다.
3. 새 규칙 아래 새 앱으로 로그인·생성·편집·삭제·완료·공유를 기기에서 검증한 뒤 앱을 배포한다. 구버전 잔존과 기존 오프라인 요청의 재연결 결과도 함께 기록한다.
