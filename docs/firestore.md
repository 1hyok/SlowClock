...
2026-09-06 19:08 KST 운영 전환: 사용자 4건·일정 26건에서 공유 코드 소유 관계를 점검했다. 소유자 충돌은 없었고 누락된 등록부 4건을 읽기·쓰기 transaction 및 `exists=false` 사전조건으로 생성했다. 기존 사용자·일정·공유 코드는 보존했다. 공개 프로필 1건에 남아 있던 `shareCode` 필드는 문서 updateTime 사전조건과 필드 마스크로 제거했다.

규칙·transaction 에뮬레이터 테스트 62개 통과 후 main `c00e38f`의 규칙을 운영에 배포했고, Rules API 재조회에서 원문 SHA-256 `ba96459964e412a068d2f8a4550fc996aecf517790c6f9082a78419c4bd0375c` 일치를 확인했다. 등록부 4건의 소유 관계도 재조회했고 공개 프로필의 공유 코드 잔존은 0건이다. 기존 일정 인덱스는 READY여서 새 인덱스 생성은 필요하지 않았다. 결제 연결은 변경하지 않았다.

이 결과는 관리자 API로 확인한 운영 데이터·규칙 적용 증거다. 사용자 요청으로 실기기 로그인·일정·공유 검증은 미뤘으며, Functions의 새 data-only 배포는 #102의 과금 제약 때문에 수행하지 않았다. 전체 앱 출시나 운영 클라이언트 검증 완료를 뜻하지 않는다. 이후 전환 때도 다음 순서를 따른다.
