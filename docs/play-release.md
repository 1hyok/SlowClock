...
| 타겟층 | 18세 이상만 선택. 어린이 대상 아님 | 어르신·보호자용 앱. 추천 화면의 「학생」 묶음(자습·공부량 확인·휴식하기·복습하기)을 이 선언과 대조했다(#220). 넷 다 성인 학습자도 그대로 쓰는 말이고 교복·학교·학년처럼 미성년을 가리키는 표현, 아동 대상 소재, 학교 일과 전제가 없어 문구를 바꾸지 않았다 |
...
휴대전화 스크린샷은 2~8장이다. 현재 준비한 4장은 실제 화면 코드의 Preview를 Linux CI에서 1080×1920(9:16)으로 렌더한다. 계정에 접속하거나 운영 일정을 만들 필요가 없다. 이미지 위치·순서·재생성 절차는 [스토어 등록 소재](play/listing.md#스토어-등록-정보-자료)를 따른다.

[Play 공식 규격](https://support.google.com/googleplay/android-developer/answer/9866151?hl=en-GB)은 JPEG 또는 24비트 PNG(알파 없음), 짧은 변 320px 이상·긴 변 3840px 이하·긴 변이 짧은 변의 2배 이하를 요구한다. 앱 홍보에는 1080px 이상과 세로 9:16을 권장한다. 이전 1080×2400(9:20) 안내는 이 비율 요건에 맞지 않아 교체했다.

추가 동작 증빙이 필요하면 전용 에뮬레이터와 테스트 데이터만 사용한다. 개인 정보가 보이는 화면은 제외한다. Play Console 업로드·미리보기 확인과 실물 기기 점검은 별도 단계다.
